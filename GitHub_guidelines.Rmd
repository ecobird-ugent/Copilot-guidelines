...
# FINDING A CP
- When the MR is looking for a CP, the MR already specifies the research idea of the project and the required skills
...
- Moreover, the MR should involve the CP after each important step of the project (i.e., checking the research question and the hypotheses, checking the finalized experimental design, as well as checking the experimental code before data collection (see below) and the analysis code before data analysis (see below))
...
- Step 2b: Add the folders as described in the lab's data management guidelines (DMG).
...
- Note that the main (and official) code checking is done via Github; however, during the programming phase, MR and CP sometimes share code snippets via slack to help each other with the programming.
...
- We are currently setting up a lab-library on Github, in which we share code that has been checked and tested in order to provide a common knowledge base for all lab members. 
